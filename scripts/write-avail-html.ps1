  ...
  @if (isLoading()) {
                ...
                >
                  ...
                  <div class="avail-page__day-header">
                    <label class="avail-page__toggle" [for]="'day-active-' + di">
                      ...
                      />
                      <span class="avail-page__toggle-track"></span>
    ...
    @if (activeTab() === 'absences') {
      <div class="avail-page__panel">
        ...
        <app-api-feedback severity="info" [message]="absenceSuccessMessage()" />

        @if (recentAbsences().length === 0) {
          <app-page-state
            kind="empty"
            message="No hay ausentismos registrados en los ultimos 60 dias."
          />
        } @else {
      ...
      <form class="modal__form" [formGroup]="absenceForm" (ngSubmit)="submitAbsence()" novalidate>
        <fieldset [disabled]="isSavingAbsence()">
          <div class="modal__grid">
            <div class="avail-page__field">
              <label for="modal-startsAt">Fecha y hora de inicio <span class="avail-page__required">*</span></label>
              <input id="modal-startsAt" type="datetime-local" formControlName="startsAt" />
              @if (showAbsenceControlError('startsAt', 'required')) {
                <p class="avail-page__error">La fecha de inicio es obligatoria.</p>
              }
            </div>

            <div class="avail-page__field">
              <label for="modal-endsAt">Fecha y hora de fin <span class="avail-page__required">*</span></label>
              <input id="modal-endsAt" type="datetime-local" formControlName="endsAt" />
              @if (showAbsenceControlError('endsAt', 'required')) {
            ...
            <div class="avail-page__field modal__field--wide">
              ...
              <textarea id="modal-reason" rows="3" formControlName="reason" placeholder="Opcional — vacaciones, cita medica, etc."></textarea>
