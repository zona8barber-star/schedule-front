...
export class StaffAvailabilityPageComponent {
  ...
  readonly absenceSuccessMessage = signal<string | null>(null);
  ...
  readonly durationWarning = computed(() => {
    ...
    return d <= 0
      ? 'La duracion base de citas es 0. Actualizala en Mi Perfil antes de guardar tu disponibilidad.'
      : null;
  });
  ...
  readonly hasRulesErrors = computed(() => {
  ...
  async saveRules(): Promise<void> {
    ...
    if (this.durationWarning()) {
      this.rulesErrorMessage.set(this.durationWarning());
      return;
    }
    ...
    try {
      ...
      for (const dayGroup of this.daysArray.controls) {
        ...
        for (const block of blocks) {
          request.push({ dayOfWeek, startTime: block.startTime, endTime: block.endTime, isActive: true });
        }
      }
  ...
  openAbsenceModal(period?: UnavailablePeriod): void {
    ...
    this.absenceSuccessMessage.set(null);

    if (period) {
      this.editingAbsenceId.set(period.id);
      this.absenceForm.patchValue({
        startsAt: formatDateTimeLocal(period.startsAtUtc),
        endsAt: formatDateTimeLocal(period.endsAtUtc),
        reason: period.reason ?? '',
      });
    } else {
      this.editingAbsenceId.set(null);
      this.absenceForm.reset({ startsAt: '', endsAt: '', reason: '' }, { emitEvent: false });
    }
  ...
  async submitAbsence(): Promise<void> {
    ...
    const request = {
      startsAtUtc: toUtcIsoString(formValue.startsAt),
      endsAtUtc: toUtcIsoString(formValue.endsAt),
  ...
  showAbsenceControlError(
    controlName: 'startsAt' | 'endsAt' | 'reason',
    errorName: string,
  ): boolean {
  ...
  showAbsenceGroupError(errorName: 'timeRange'): boolean {
    return (
...
// ── Validators ─────────────────────────────────────────────────────────────
function absenceRangeValidator(): ValidatorFn {
  return (control: AbstractControl): ValidationErrors | null => {
    const group = control as AbsenceFormGroup;
    const startsAt = group.controls.startsAt.value;
    const endsAt = group.controls.endsAt.value;
    if (!startsAt || !endsAt) return null;
    return new Date(startsAt).getTime() < new Date(endsAt).getTime() ? null : { timeRange: true };
  };
}
