$dest = "d:\Proyects\barbershop\app\front\barbershop-pwa\src\app\features\staff\profile\pages\staff-profile-page\staff-profile-page.component.scss"
...
.profile-page__thumbnail-wrap {
  width: 100%;
  height: 140px;
  border-radius: 0.75rem;
  overflow: hidden;
  border: 1px solid var(--border-soft, rgba(110, 96, 74, 0.3));
  background: rgba(22, 24, 23, 0.4);
  display: flex;
  align-items: center;
...
.profile-page__thumbnail {
  width: 100%;
  height: 100%;
  object-fit: cover;
}

.profile-page__media-actions {
  display: flex;
  gap: 0.6rem;
  flex-wrap: wrap;
}

.profile-page__btn-upload {
  flex: 1;
  padding: 0.5rem 1rem;
  ...
  &:hover:not(:disabled) {
    ...
    border-color: rgba(176, 138, 74, 0.8);
  }
...
.profile-page__btn-remove {
  padding: 0.5rem 1rem;
  border: 1px solid rgba(180, 83, 60, 0.5);
  border-radius: 0.75rem;
  ...
  &:hover:not(:disabled) {
    background: rgba(180, 83, 60, 0.18);
  }
...
.profile-page__section-title {
  ...
  border-bottom: 1px solid var(--border-soft, rgba(110, 96, 74, 0.25));
}
...
.profile-page__field {
  ...
  textarea {
    ...
    &.ng-invalid.ng-touched {
      border-color: rgba(180, 83, 60, 0.7);
    }
  }
...
[System.IO.File]::WriteAllText($dest, $scss, [System.Text.UTF8Encoding]::new($false))
